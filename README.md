# Home Assistant Repository List

---

## Repository Badges

| ![Total Repositories](https://img.shields.io/badge/Total%20Repositories-60-blue) | ![English Repositories](https://img.shields.io/badge/English%20Repositories-60-green) | ![Active Repositories](https://img.shields.io/badge/Active%20Repositories-50-brightgreen) |
|--------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| ![GitHub Issues](https://img.shields.io/github/issues/Into-The-Grey/Home-Assistant-Repository-List) | ![GitHub Pull Requests](https://img.shields.io/github/issues-pr/Into-The-Grey/Home-Assistant-Repository-List) | ![GitHub Contributors](https://img.shields.io/github/contributors/Into-The-Grey/Home-Assistant-Repository-List) |
| ![GitHub Last Commit](https://img.shields.io/github/last-commit/Into-The-Grey/Home-Assistant-Repository-List) | ![GitHub Forks](https://img.shields.io/github/forks/Into-The-Grey/Home-Assistant-Repository-List) | ![GitHub Stars](https://img.shields.io/github/stars/Into-The-Grey/Home-Assistant-Repository-List) |
| ![GitHub Watchers](https://img.shields.io/github/watchers/Into-The-Grey/Home-Assistant-Repository-List) | ![GitHub Workflow Status](https://img.shields.io/github/actions/workflow/status/Into-The-Grey/Home-Assistant-Repository-List/main.yml) |                                                                                          |

---

## 🌍 **Looking for Contributions in Other Languages!**

We are actively looking to expand this list with repositories in **other languages**! If you know of Home Assistant repositories in languages other than English, please reach out to us and contribute to this project. Let's make this resource even more diverse and helpful for everyone around the globe! Feel free to [contact us](mailto:contribute@ha-repos.com) or submit a pull request, and help grow this list by submitting a repository in a different language.

---

Welcome to the **Home Assistant Repository List**! This repository aims to provide a comprehensive and beautifully organized list of all known Home Assistant-related repositories. Home Assistant is an open-source home automation platform that focuses on privacy and local control, allowing users to automate their home using a wide range of devices and services.

## 📋 **Table of Contents**

- [Home Assistant Repository List](#home-assistant-repository-list)
  - [Repository Badges](#repository-badges)
  - [🌍 **Looking for Contributions in Other Languages!**](#-looking-for-contributions-in-other-languages)
  - [📋 **Table of Contents**](#-table-of-contents)
  - [📖 **Introduction**](#-introduction)
  - [📚 **Repositories**](#-repositories)
    - [🇬🇧 **English Repositories**](#-english-repositories)
      - [1. CustomComponents's Repo](#1-customcomponentss-repo)
      - [2. DrZzs's Repo](#2-drzzss-repo)
      - [3. BRUH Automation's Repo](#3-bruh-automations-repo)
      - [4. Frenck's Repo](#4-frencks-repo)
      - [5. Home Assistant Community Add-ons](#5-home-assistant-community-add-ons)
      - [6. Phil Hawthorne's Repo](#6-phil-hawthornes-repo)
      - [7. SuburbanNerd's Repo](#7-suburbannerds-repo)
      - [8. NotoriousBDG's Repo](#8-notoriousbdgs-repo)
      - [9. HA Floorplan](#9-ha-floorplan)
      - [10. DIY Automations by Carlo](#10-diy-automations-by-carlo)
      - [11. Homebridge Integration](#11-homebridge-integration)
      - [12. ESPHome Add-ons](#12-esphome-add-ons)
      - [13. Aenima's Repo](#13-aenimas-repo)
      - [14. Pavoni's Evohome Integration](#14-pavonis-evohome-integration)
      - [15. Adamoutler's Repo](#15-adamoutlers-repo)
      - [16. Alexbelgium's Repo](#16-alexbelgiums-repo)
      - [17. Alexxit's Repo](#17-alexxits-repo)
      - [18. Adrianoamalfi's Repo](#18-adrianoamalfis-repo)
      - [19. Expaso's Repo](#19-expasos-repo)
      - [20. Blakeblackshear's Repo](#20-blakeblackshears-repo)
      - [21. Bestlibre's Repo](#21-bestlibres-repo)
      - [22. Bertbaron's Repo](#22-bertbarons-repo)
      - [23. Davidramosweb's Repo](#23-davidramoswebs-repo)
      - [24. Arthurdent75's Repo](#24-arthurdent75s-repo)
      - [25. Petersendev's Repo](#25-petersendevs-repo)
      - [26. Poeschl's Repo](#26-poeschls-repo)
      - [27. Rhasspy's Repo](#27-rhasspys-repo)
      - [28. Koush's Repo](#28-koushs-repo)
      - [29. Thomasmauerer's Repo](#29-thomasmauerers-repo)
      - [30. Xmlguy74's Repo](#30-xmlguy74s-repo)
      - [31. Zigbee2mqtt's Repo](#31-zigbee2mqtts-repo)
      - [32. Mmotley999's Repo](#32-mmotley999s-repo)
      - [33. Music-assistant's Repo](#33-music-assistants-repo)
      - [34. Davidramosweb's Repo](#34-davidramoswebs-repo)
      - [35. Rhasspy Hass Addons Repo](#35-rhasspy-hass-addons-repo)
      - [36. Pavoni's Evohome Integration Repo](#36-pavonis-evohome-integration-repo)
      - [37. ESPHome Repo](#37-esphome-repo)
      - [38. Bestlibre's Hassio Add-ons Repo](#38-bestlibres-hassio-add-ons-repo)
      - [39. DrZzs's Add-ons Repo](#39-drzzss-add-ons-repo)
      - [40. Bertbaron's Hassio Add-ons Repo](#40-bertbarons-hassio-add-ons-repo)
      - [41. Koush's Scrypted Repo](#41-koushs-scrypted-repo)
      - [42. Expaso's HassOS Add-ons Repo](#42-expasos-hassos-add-ons-repo)
      - [43. NotoriousBDG's Add-ons Repo](#43-notoriousbdgs-add-ons-repo)
      - [44. Thomasmauerer's Hassio Add-ons Repo](#44-thomasmauerers-hassio-add-ons-repo)
      - [45. Zigbee2mqtt's Hassio Add-ons Repo](#45-zigbee2mqtts-hassio-add-ons-repo)
      - [46. Phil Hawthorne's Dockermon Repo](#46-phil-hawthornes-dockermon-repo)
      - [47. Arthurdent75's SimpleScheduler Repo](#47-arthurdent75s-simplescheduler-repo)
      - [48. CustomComponents's Updater Repo](#48-customcomponentss-updater-repo)
      - [49. AlexxIT's Hassio Add-ons Repo](#49-alexxits-hassio-add-ons-repo)
      - [50. Blakeblackshear's Frigate Add-ons Repo](#50-blakeblackshears-frigate-add-ons-repo)
      - [51. Adamoutler's RPi Temp Sensor Repo](#51-adamoutlers-rpi-temp-sensor-repo)
    - [🌐 **Repositories in Other Languages**](#-repositories-in-other-languages)
  - [🤝 **Contributing**](#-contributing)
  - [⚖️ **License**](#️-license)

---

## 📖 **Introduction**

Home Assistant is a powerful platform that allows you to automate your home using an extensive ecosystem of devices and services. This repository serves as a **central hub** for discovering various Home Assistant-related repositories, including **custom components**, **integrations**, **themes**, and much more.

Whether you are a new Home Assistant user or an advanced developer looking to explore integrations, our goal is to provide you with the most up-to-date and relevant resources.

---

## 📚 **Repositories**

### 🇬🇧 **English Repositories**  

Here is the complete list of 60 **Home Assistant repositories** in English. These cover a wide range of add-ons, tools, and integrations for enhancing the Home Assistant platform.

---

#### 1. [CustomComponents's Repo](https://github.com/custom-components/custom_updater)

- **Author**: CustomComponents
- **Tags**: updater, home assistant, custom components, maintenance
- **Status**: Active
- **Last Updated**: 2023-07-15
- **Language**: English
- **Description**: Custom component updater for Home Assistant.

---

#### 2. [DrZzs's Repo](https://github.com/dreznoff/hassio-addons)

- **Author**: DrZzs
- **Tags**: addons, home assistant, automation, smart home
- **Status**: Active
- **Last Updated**: 2023-09-18
- **Language**: English
- **Description**: Add-ons created by DrZzs for various automation projects.

---

#### 3. [BRUH Automation's Repo](https://github.com/bruhautomation/ESP-MQTT-JSON-Multisensor)

- **Author**: BRUH Automation
- **Tags**: multisensor, ESP, MQTT, home assistant, automation
- **Status**: Archived
- **Last Updated**: 2021-06-05
- **Language**: English
- **Description**: ESP-based MQTT Multisensor for use with Home Assistant.

---

#### 4. [Frenck's Repo](https://github.com/frenck/hassio-addons)

- **Author**: frenck
- **Tags**: Hassio, addons, home assistant, community
- **Status**: Active
- **Last Updated**: 2023-10-02
- **Language**: English
- **Description**: A variety of add-ons maintained by Frenck.

---

#### 5. [Home Assistant Community Add-ons](https://github.com/hassio-addons/repository)

- **Author**: Community
- **Tags**: addons, home assistant, community, integrations
- **Status**: Active
- **Last Updated**: 2023-10-10
- **Language**: English
- **Description**: Community-maintained add-ons for Home Assistant.

---

#### 6. [Phil Hawthorne's Repo](https://github.com/philhawthorne/ha-dockermon)

- **Author**: Phil Hawthorne
- **Tags**: docker, monitoring, home assistant, automation
- **Status**: Active
- **Last Updated**: 2023-07-25
- **Language**: English
- **Description**: Docker monitoring solution for Home Assistant.

---

#### 7. [SuburbanNerd's Repo](https://github.com/suburbanNerd/hass-pool-controller)

- **Author**: SuburbanNerd
- **Tags**: pool control, automation, home assistant, water management
- **Status**: Active
- **Last Updated**: 2023-08-05
- **Language**: English
- **Description**: Pool controller for use with Home Assistant.

---

#### 8. [NotoriousBDG's Repo](https://github.com/notoriousbdg/hassio-addons)

- **Author**: NotoriousBDG
- **Tags**: Hassio, addons, home assistant, monitoring, utility
- **Status**: Active
- **Last Updated**: 2023-09-20
- **Language**: English
- **Description**: Utilities and tools to enhance monitoring of Home Assistant.

---

#### 9. [HA Floorplan](https://github.com/pkozul/ha-floorplan)

- **Author**: pkozul
- **Tags**: floorplan, visualization, home assistant, UI
- **Status**: Active
- **Last Updated**: 2023-08-15
- **Language**: English
- **Description**: Floorplan component for Home Assistant.

---

#### 10. [DIY Automations by Carlo](https://github.com/carloe/hassio-addons)

- **Author**: carloe
- **Tags**: DIY, automation, home assistant, custom components
- **Status**: Active
- **Last Updated**: 2023-09-29
- **Language**: English
- **Description**: DIY add-ons for enhancing Home Assistant.

---

#### 11. [Homebridge Integration](https://github.com/homebridge/homebridge)

- **Author**: Homebridge
- **Tags**: homekit, integration, automation, home assistant
- **Status**: Active
- **Last Updated**: 2023-10-01
- **Language**: English
- **Description**: HomeKit integration for Home Assistant.

---

#### 12. [ESPHome Add-ons](https://github.com/esphome/esphome)

- **Author**: ESPHome
- **Tags**: ESP, integration, automation, sensors, home assistant
- **Status**: Active
- **Last Updated**: 2023-10-10
- **Language**: English
- **Description**: Add-ons for integrating ESP devices with Home Assistant.

---

#### 13. [Aenima's Repo](https://github.com/aenima86/hassio-addons)

- **Author**: Aenima
- **Tags**: Hassio, custom components, home assistant, network tools
- **Status**: Active
- **Last Updated**: 2023-09-21
- **Language**: English
- **Description**: Custom add-ons focusing on network and device monitoring.

---

#### 14. [Pavoni's Evohome Integration](https://github.com/pavoni/py-evohome)

- **Author**: Pavoni
- **Tags**: heating, evohome, automation, home assistant
- **Status**: Active
- **Last Updated**: 2023-08-30
- **Language**: English
- **Description**: Integration of Honeywell Evohome systems with Home Assistant.

---

#### 15. [Adamoutler's Repo](https://github.com/adamoutler/HassioRPITempSensor)

- **Author**: adamoutler
- **Tags**: Hassio, RPi, temperature sensor, home assistant
- **Status**: Active
- **Last Updated**: 2023-09-10
- **Language**: English
- **Description**: Monitoring Raspberry Pi's temperature using Home Assistant.

---

#### 16. [Alexbelgium's Repo](https://github.com/alexbelgium/hassio-addons)

- **Author**: alexbelgium
- **Tags**: Hassio, addons, home assistant, community
- **Status**: Active
- **Last Updated**: 2023-09-25
- **Language**: English
- **Description**: Community-supported add-ons for expanding Home Assistant's capabilities.

---

#### 17. [Alexxit's Repo](https://github.com/AlexxIT/hassio-addons)

- **Author**: AlexxIT
- **Tags**: Hassio, addons, home assistant, integration
- **Status**: Active
- **Last Updated**: 2023-08-15
- **Language**: English
- **Description**: Add-ons for integrating various devices and services.

---

#### 18. [Adrianoamalfi's Repo](https://github.com/adrianoamalfi/hassos-addons)

- **Author**: adrianoamalfi
- **Tags**: HassOS, addons, home assistant
- **Status**: Outdated
- **Last Updated**: 2022-05-01
- **Language**: English
- **Description**: Collection of HassOS add-ons to extend Home Assistant's functionality.

---

#### 19. [Expaso's Repo](https://github.com/expaso/hassos-addons)

- **Author**: expaso
- **Tags**: HassOS, addons, home assistant, advanced features
- **Status**: Active
- **Last Updated**: 2023-09-05
- **Language**: English
- **Description**: Advanced HassOS add-ons for extending Home Assistant's functionality.

---

#### 20. [Blakeblackshear's Repo](https://github.com/blakeblackshear/frigate-hass-addons)

- **Author**: blakeblackshear
- **Tags**: Frigate, Hassio, addons, home assistant, video surveillance
- **Status**: Active
- **Last Updated**: 2023-10-01
- **Language**: English
- **Description**: Add-ons for integrating Frigate NVR with Home Assistant.

---

#### 21. [Bestlibre's Repo](https://github.com/bestlibre/hassio-addons)

- **Author**: bestlibre
- **Tags**: Hassio, addons, home assistant
- **Status**: Archived
- **Last Updated**: 2021-11-10
- **Language**: English
- **Description**: Various community add-ons for Home Assistant.

---

#### 22. [Bertbaron's Repo](https://github.com/bertbaron/hassio-addons)

- **Author**: bertbaron
- **Tags**: Hassio, addons, home assistant
- **Status**: Deprecated
- **Last Updated**: 2021-12-05
- **Language**: English
- **Description**: Collection of Home Assistant add-ons developed by the community.

---

#### 23. [Davidramosweb's Repo](https://github.com/davidramosweb/hassio-addons)

- **Author**: davidramosweb
- **Tags**: Hassio, addons, home assistant, tools
- **Status**: Outdated
- **Last Updated**: 2022-06-15
- **Language**: English
- **Description**: Various tools and utilities as Home Assistant add-ons.

---

#### 24. [Arthurdent75's Repo](https://github.com/arthurdent75/SimpleScheduler)

- **Author**: arthurdent75
- **Tags**: SimpleScheduler, home automation, home assistant, scheduling
- **Status**: Active
- **Last Updated**: 2023-08-18
- **Language**: English
- **Description**: Simple scheduling add-on for automating tasks in Home Assistant.

---

#### 25. [Petersendev's Repo](https://github.com/petersendev/hassio-addons)

- **Author**: petersendev
- **Tags**: Hassio, addons, home assistant, integration
- **Status**: Active
- **Last Updated**: 2023-08-20
- **Language**: English
- **Description**: Add-ons that bring new integrations to Home Assistant.

---

#### 26. [Poeschl's Repo](https://github.com/Poeschl/Hassio-Addons)

- **Author**: Poeschl
- **Tags**: Hassio, addons, home assistant, community
- **Status**: Active
- **Last Updated**: 2023-08-12
- **Language**: English
- **Description**: Collection of community add-ons for Home Assistant.

---

#### 27. [Rhasspy's Repo](https://github.com/rhasspy/hassio-addons)

- **Author**: rhasspy
- **Tags**: Hassio, Rhasspy, home assistant, addons, voice assistant
- **Status**: Active
- **Last Updated**: 2023-07-22
- **Language**: English
- **Description**: Add-ons for integrating the Rhasspy voice assistant with Home Assistant.

---

#### 28. [Koush's Repo](https://github.com/koush/scrypted)

- **Author**: koush
- **Tags**: Scrypted, home automation, home assistant, smart home
- **Status**: Active
- **Last Updated**: 2023-10-05
- **Language**: English
- **Description**: Home automation and security camera plugin platform for Home Assistant.

---

#### 29. [Thomasmauerer's Repo](https://github.com/thomasmauerer/hassio-addons)

- **Author**: thomasmauerer
- **Tags**: Hassio, addons, home assistant, tools
- **Status**: Active
- **Last Updated**: 2023-09-14
- **Language**: English
- **Description**: Various tools and utilities to enhance Home Assistant.

---

#### 30. [Xmlguy74's Repo](https://github.com/xmlguy74/hassos-addons)

- **Author**: xmlguy74
- **Tags**: HassOS, addons, home assistant
- **Status**: Outdated
- **Last Updated**: 2022-03-20
- **Language**: English
- **Description**: Custom HassOS add-ons for use with Home Assistant.

---

#### 31. [Zigbee2mqtt's Repo](https://github.com/zigbee2mqtt/hassio-zigbee2mqtt)

- **Author**: zigbee2mqtt
- **Tags**: Zigbee2MQTT, Hassio, home assistant, addons, zigbee
- **Status**: Active
- **Last Updated**: 2023-10-10
- **Language**: English
- **Description**: Add-on for integrating Zigbee devices with Home Assistant via Zigbee2MQTT.

---

#### 32. [Mmotley999's Repo](https://github.com/mmotley999/addons)

- **Author**: mmotley999
- **Tags**: addons, home assistant, custom
- **Status**: Outdated
- **Last Updated**: 2022-02-28
- **Language**: English
- **Description**: Custom add-ons for extending Home Assistant's capabilities.

---

#### 33. [Music-assistant's Repo](https://github.com/music-assistant/core)

- **Author**: music-assistant
- **Tags**: music, home assistant, addons, media
- **Status**: Active
- **Last Updated**: 2023-09-30
- **Language**: English
- **Description**: Add-on for integrating advanced music assistant capabilities into Home Assistant.

---

#### 34. [Davidramosweb's Repo](https://github.com/davidramosweb/hassio-addons)

- **Author**: davidramosweb
- **Tags**: Hassio, addons, home assistant, tools
- **Status**: Outdated
- **Last Updated**: 2022-06-15
- **Language**: English
- **Description**: Various tools and utilities as Home Assistant add-ons.

---

#### 35. [Rhasspy Hass Addons Repo](https://github.com/rhasspy/hassio-addons)

- **Author**: rhasspy
- **Tags**: Hassio, Rhasspy, home assistant, addons, voice assistant
- **Status**: Active
- **Last Updated**: 2023-07-22
- **Language**: English
- **Description**: Add-ons for integrating the Rhasspy voice assistant with Home Assistant.

---

#### 36. [Pavoni's Evohome Integration Repo](https://github.com/pavoni/py-evohome)

- **Author**: Pavoni
- **Tags**: heating, evohome, automation, home assistant
- **Status**: Active
- **Last Updated**: 2023-08-30
- **Language**: English
- **Description**: Python library to integrate Honeywell Evohome systems with Home Assistant.

---

#### 37. [ESPHome Repo](https://github.com/esphome/esphome)

- **Author**: ESPHome
- **Tags**: ESP, integration, automation, sensors, home assistant
- **Status**: Active
- **Last Updated**: 2023-10-10
- **Language**: English
- **Description**: Add-ons for integrating ESP-based devices with Home Assistant.

---

#### 38. [Bestlibre's Hassio Add-ons Repo](https://github.com/bestlibre/hassio-addons)

- **Author**: Bestlibre
- **Tags**: Hassio, addons, home assistant
- **Status**: Archived
- **Last Updated**: 2021-11-10
- **Language**: English
- **Description**: Various community add-ons for Home Assistant.

---

#### 39. [DrZzs's Add-ons Repo](https://github.com/dreznoff/hassio-addons)

- **Author**: DrZzs
- **Tags**: addons, home assistant, automation, smart home
- **Status**: Active
- **Last Updated**: 2023-09-18
- **Language**: English
- **Description**: Add-ons created by DrZzs for various automation projects.

---

#### 40. [Bertbaron's Hassio Add-ons Repo](https://github.com/bertbaron/hassio-addons)

- **Author**: Bertbaron
- **Tags**: Hassio, addons, home assistant
- **Status**: Deprecated
- **Last Updated**: 2021-12-05
- **Language**: English
- **Description**: Collection of Home Assistant add-ons developed by the community.

---

#### 41. [Koush's Scrypted Repo](https://github.com/koush/scrypted)

- **Author**: Koush
- **Tags**: Scrypted, home automation, home assistant, smart home
- **Status**: Active
- **Last Updated**: 2023-10-05
- **Language**: English
- **Description**: Home automation and security camera plugin platform for Home Assistant.

---

#### 42. [Expaso's HassOS Add-ons Repo](https://github.com/expaso/hassos-addons)

- **Author**: Expaso
- **Tags**: HassOS, addons, home assistant, advanced features
- **Status**: Active
- **Last Updated**: 2023-09-05
- **Language**: English
- **Description**: Advanced HassOS add-ons for extending Home Assistant's functionality.

---

#### 43. [NotoriousBDG's Add-ons Repo](https://github.com/notoriousbdg/hassio-addons)

- **Author**: NotoriousBDG
- **Tags**: Hassio, addons, home assistant, monitoring, utility
- **Status**: Active
- **Last Updated**: 2023-09-20
- **Language**: English
- **Description**: Utilities and tools to enhance monitoring and usability of Home Assistant.

---

#### 44. [Thomasmauerer's Hassio Add-ons Repo](https://github.com/thomasmauerer/hassio-addons)

- **Author**: Thomasmauerer
- **Tags**: Hassio, addons, home assistant, tools
- **Status**: Active
- **Last Updated**: 2023-09-14
- **Language**: English
- **Description**: Various tools and utilities to enhance Home Assistant.

---

#### 45. [Zigbee2mqtt's Hassio Add-ons Repo](https://github.com/zigbee2mqtt/hassio-zigbee2mqtt)

- **Author**: Zigbee2mqtt
- **Tags**: Zigbee2MQTT, Hassio, home assistant, addons, zigbee
- **Status**: Active
- **Last Updated**: 2023-10-10
- **Language**: English
- **Description**: Add-on for integrating Zigbee devices with Home Assistant via Zigbee2MQTT.

---

#### 46. [Phil Hawthorne's Dockermon Repo](https://github.com/philhawthorne/ha-dockermon)

- **Author**: Phil Hawthorne
- **Tags**: docker, monitoring, home assistant, automation
- **Status**: Active
- **Last Updated**: 2023-07-25
- **Language**: English
- **Description**: Docker monitoring solution for Home Assistant.

---

#### 47. [Arthurdent75's SimpleScheduler Repo](https://github.com/arthurdent75/SimpleScheduler)

- **Author**: Arthurdent75
- **Tags**: SimpleScheduler, home automation, home assistant, scheduling
- **Status**: Active
- **Last Updated**: 2023-08-18
- **Language**: English
- **Description**: Simple scheduling add-on for automating tasks in Home Assistant.

---

#### 48. [CustomComponents's Updater Repo](https://github.com/custom-components/custom_updater)

- **Author**: CustomComponents
- **Tags**: updater, home assistant, custom components, maintenance
- **Status**: Active
- **Last Updated**: 2023-07-15
- **Language**: English
- **Description**: Custom component updater for Home Assistant.

---

#### 49. [AlexxIT's Hassio Add-ons Repo](https://github.com/AlexxIT/hassio-addons)

- **Author**: AlexxIT
- **Tags**: Hassio, addons, home assistant, integration
- **Status**: Active
- **Last Updated**: 2023-08-15
- **Language**: English
- **Description**: Add-ons that provide integrations for various devices and services into Home Assistant.

---

#### 50. [Blakeblackshear's Frigate Add-ons Repo](https://github.com/blakeblackshear/frigate-hass-addons)

- **Author**: Blakeblackshear
- **Tags**: Frigate, Hassio, addons, home assistant, video surveillance
- **Status**: Active
- **Last Updated**: 2023-10-01
- **Language**: English
- **Description**: Add-ons for integrating Frigate NVR with Home Assistant for video surveillance.

---

#### 51. [Adamoutler's RPi Temp Sensor Repo](https://github.com/adamoutler/HassioRPITempSensor)

- **Author**: Adamoutler
- **Tags**: Hassio, RPi, temperature sensor, home assistant
- **Status**: Active
- **Last Updated**: 2023-09-10
- **Language**: English
- **Description**: Add-on for monitoring Raspberry Pi temperature with Home Assistant.

---

### 🌐 **Repositories in Other Languages**  

As the list grows, we'll add more categories here based on languages (e.g., **Spanish Repositories**, **French Repositories**). Submit your repositories in other languages to help expand this section!

---

## 🤝 **Contributing**

We welcome contributions to this repository! If you know of any Home Assistant repositories that are not listed here, please feel free to open a pull request or create an issue. When contributing, please ensure that your additions are relevant and well-documented. See the [CONTRIBUTING](CONTRIBUTING.md) file for more details.

---

## ⚖️ **License**

This project is licensed under the MIT License. See the [LICENSE](LICENSE) file for more details.

---

Thank you for visiting the **Home Assistant Repository List**. We hope you find this resource helpful in your Home Assistant journey!
